...
export default function TradesPage() {
  ...
  return (
    <div className="bg-[#F4F8FC] text-[#1F2C56] py-12 px-4 sm:px-6 md:px-8">
      ...
      <div className="bg-[#F4F8FC] py-10 px-4 sm:px-6">
        <h3 className="text-3xl sm:text-4xl font-extrabold text-center text-[#1F2C56] mb-8">
          Admission Process
        </h3>
          ...
          {steps.map((item, index, arr) => {
            ...
            return (
              <div key={index} className="relative z-10">
                {!isFirst && (
                  <div
                    className={`absolute top-0 w-0 h-0 border-l-[15px] border-l-transparent border-r-[15px] border-r-transparent border-t-[30px] border-t-[#F4F8FC] ${
                      isEven ? 'left-10 sm:left-28' : 'right-10 sm:right-28'
                    }`}
                  />
                )}

                <div className="bg-[#102645] text-white px-6 py-8 sm:px-10 rounded-lg shadow-lg flex flex-col sm:flex-row items-center gap-6">
                  <div className="text-[24px] sm:text-[28px] font-bold">
                    {item.step}
                  </div>
                  <div className="w-full text-center sm:text-left">
                    <h3 className="text-yellow-500 font-bold text-[18px] sm:text-[20px] uppercase tracking-wide underline mb-1">
                      {item.title}
                    </h3>
                    <p className="text-white text-[16px] sm:text-[18px] leading-snug">
                      {item.description}
                    </p>
                  </div>
                </div>
                ...
                {!isLast && (
                  <div className="relative z-10">
                    <div
                      className={`absolute bottom-[-30px] w-0 h-0 border-l-[30px] border-l-transparent border-r-[30px] border-r-transparent border-t-[50px] border-t-[#102645] ${
      ...
      <div className="w-full px-2">
        <div className="mx-auto max-w-[1400px] overflow-x-auto rounded-lg shadow-lg">
          <table className="w-full text-sm sm:text-base md:text-lg text-center border-separate" style={{ borderSpacing: 0 }}>
            ...
            <tbody className="bg-[#BAC7E5] text-black font-semibold">
              {[
                [<>Computer Operator and <br /> Programming Assistant (COPA)</>, "Non-Engineering", "10th Passed", "1 Year"],
                [<>Refrigeration and <br /> Air Conditioning Technician</>, "Engineering", "10th Passed", "2 Years"],
